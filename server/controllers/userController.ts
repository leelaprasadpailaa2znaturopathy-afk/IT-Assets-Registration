...
export async function createUser(req: AuthRequest, res: Response) {
    try {
        ...
        const newUser = await User.create({
            ...
            passwordHash: "temp", // Will be replaced below via pre-save is not triggered, so hash manually
            role: role || "User",
            activeStatus: typeof activeStatus === "boolean" ? activeStatus : true,
        });

        // Hash password via model method if available, or use bcrypt directly here
        const { default: bcryptjs } = await import("bcryptjs");
        const salt = await bcryptjs.genSalt(10);
        newUser.passwordHash = await bcryptjs.hash(password, salt);
        await newUser.save();
